...
| 8 | ✅ GPU node (Phase 3) | **Done 2026-09-03 on your explicit go-ahead** — and it was not a re-provision. Talos adds system extensions through an upgrade (`wipe: false`), so it cost one reboot rather than a rebuild: `space-crucible-prod` was down for about two minutes and came back green. A fresh CNPG backup was taken first anyway. The driver is the **lts** branch (580.173.02), because production is on 595 and 580 is the last one that supports Pascal |
